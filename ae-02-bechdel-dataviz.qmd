...
Each observation represents a **A movie**.
...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013))+
  ...
  labs(
    x = "2013 Budget ($)", 
    y = "Gross Budget ($)", 
    title = "Gross Revenue vs Budget in 2013"
  )
```
...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013, colour = binary, size= roi)) +
  ...
  labs(
    x = "Budget (in 2013 $)", 
    y = "Gross revenue (in 2013 $)", 
    title = "Gross revenue vs. budget, by Bechdel test results scaled by ROI"
  )
```
...
ggplot(bechdel, aes(x = budget_2013/1000000, y = gross_2013/1000000, colour = binary, size= roi, alpha =0.5)) +
  geom_point() + 
  scale_x_continuous(breaks = pretty_breaks(6), expand = c(0,100)) +
  scale_y_continuous(breaks = pretty_breaks(6))+
  facet_wrap(~ clean_test) +
  labs(
    x = "Budget (in 2013 Million$)", 
    y = "Gross revenue (in 2013 Million$)", 
    title = "Gross revenue vs. budget, by Bechdel test results scaled by ROI"
  ) 
```
...
ggplot(bechdel, aes(x = budget_2013/1000000, y = gross_2013/1000000, size= roi, alpha =0.5)) +
  geom_point() + 
  scale_x_continuous(breaks = pretty_breaks(6), expand = c(0,100)) +
  scale_y_continuous(breaks = pretty_breaks(6))+
...
All the passing ones are in the OK for clean_test category
...
ggplot(bechdel, aes(x = budget_2013/1000000, y = gross_2013/1000000, size= roi, alpha =0.8)) +
  geom_point(color = "firebrick") + 
  scale_x_continuous(breaks = pretty_breaks(2), expand = c(0,100)) +
  scale_y_continuous(breaks = pretty_breaks(6))+
  facet_grid(binary ~ clean_test) +
  theme_bw()
  labs(
...
ggplot(bechdel, aes(x = clean_test, y = roi, colour = binary,)) +
  geom_boxplot() + 
  labs(
    x = "Clean Test", 
    y = "ROI", 
    title = "ROI vs Clean Test Results") +
  coord_cartesian(ylim = c(0,18))
```

What are those movies with *very* high returns on investment?

Ones that pass, do slightly better, and never perform worse then the ones that do not, in the sample set.
...
Expand on your plot from the previous step to zoom in on movies with `roi < 50` to get a better view of how the medians across the categories compare.
...
bechdel |>
  filter(roi < 50) |>
  ggplot(aes(x = clean_test, y = roi, colour = binary,)) +
  geom_boxplot() + 
  labs(
    x = "Clean Test", 
    y = "ROI", 
...
The return on investment on movies that pass the Bechdel test, is in similar ranges as others, and we can see that movies that do pass do not do worse than movies that do not.
The Median ROI is slightly higher then movies that do not.
